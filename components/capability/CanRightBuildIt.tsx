...
export function CanRightBuildIt({ questions, prefix = "", context }: { questions: string[]; prefix?: string; context: string }) {
  ...
  return (
    <aside aria-label={f.sidebarTitle} className="border border-navy/15 bg-white">
      ...
      <div className="p-5">
        <CompactFinder
          ...
          placeholder={f.sidebarPlaceholder}
          button={f.sidebarButton}
          stacked
          prefix={prefix}
        />
        <p className="label mt-6 text-[0.66rem] text-steel">{dict.common.engineersAlsoAsk}</p>
        <ul className="mt-2">
          {questions.map((q) => (
            <li key={q} className="border-b border-navy/10 last:border-b-0">
              <button type="button" onClick={() => askFinder(q)} className="group flex w-full items-start justify-between gap-3 py-3 text-start text-[0.95rem] text-navy hover:text-blue">
                <span>{q}</span>
      ...
      <div className="grid grid-cols-2 border-t border-navy/10">
        <a href={company.phoneHref} className="label flex items-center justify-center gap-2 py-4 text-[0.68rem] text-navy hover:bg-mist">
          <Phone size={14} /> {dict.common.call}
        </a>
        <Link href="/start-a-project" className="label flex items-center justify-center gap-2 border-s border-navy/10 bg-blue py-4 text-[0.68rem] text-white hover:bg-blue-600">
          {dict.common.startRfq} <ArrowRight size={14} />
        </Link>
      </div>
    </aside>
  );
