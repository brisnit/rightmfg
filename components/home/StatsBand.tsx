...
export async function StatsBand() {
  ...
  const { customers, headlineStats, teamStats } = content;
  return (
      ...
      <div className="container-x">
        <Reveal plain className="grid grid-cols-2 lg:grid-cols-4">
          {headlineStats.map((s, i) => (
            <div
              key={s.label}
              className={`relative border-navy/12 py-10 sm:py-14 lg:py-20 ${i % 2 === 1 ? "border-s ps-5 sm:ps-8" : "pe-4"} ${i > 1 ? "border-t lg:border-t-0" : ""} ${i === 2 ? "lg:border-s lg:ps-8" : ""} ${i === 1 || i === 3 ? "" : ""}`}
            >
              <span className="rule-draw absolute start-0 end-0 top-0 h-[3px] bg-blue" style={{ ["--d" as string]: `${i * 120}ms` }} aria-hidden />
              <p className="label tabular text-steel">{String(i + 1).padStart(2, "0")}</p>
              <p className="display mt-4 text-[clamp(3rem,5.7vw,6.4rem)] leading-[0.85] text-navy">
        ...
        <div className="grid gap-px border-y border-navy/12 bg-navy/12 sm:grid-cols-3">
          {teamStats.map((s) => (
            <div key={s.label} className="flex items-baseline gap-4 bg-paper py-5 sm:px-6 sm:first:ps-0">
        ...
        <div className="py-12 lg:py-16">
          <p className="label text-steel">{dict.stats.trustedBy}</p>
